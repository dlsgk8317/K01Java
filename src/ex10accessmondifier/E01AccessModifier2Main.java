...
import ex10accessmondifier.sub.E01AccessModifier2;

public class E01AccessModifier2Main {
	...
	public static void main(String[] args) {
		...
		-private로 선언된 멤버는 동일한 패키지라 할지라도 접근할수 없다.
		
		※private으로 선언된 멤버에 외부클래스에서 접근하려면 public으로 선언된
		멤버메소드를 통해 간접적으로 접근해야한다.
		 */
...
//		one.privateMethod(); //에러발생
		...
		System.out.println("E01ccessModifier2 객체 생성 및 접근");
		/*
...
//		System.out.println("tow.defaultvar=" + two.defaultVar); //에러발생
		System.out.println("tow.publicVar=" + two.publicVar);
...
//		two.defaultMethod(); //에러발생
		...
		/*
		다른 패키지에서 선언된 디폴트 클래스이므로 객체 생성이 불가능하다.
		import는 불가능하므로 아예 사용할 수 없는 클래스가된다.
		 */
		System.out.println("DefaultClass 객체 생성 및 접근");
