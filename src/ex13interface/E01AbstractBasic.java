...
class AbsClassChild1 extends AbsClass1 {
	...
	void newMethod() {
		...
		강제사항이 아닌 선택사항이다. 즉, 필요한 경우에만 오버라이딩 한다.
		 */
...
추상클래스2 : 추상메소드를 포함한 클래스
	추상메소드를 멤버로 포함한 클래스는 반드시 abstract로 선언해야한다. 그렇지않으면 에러가 발생한다.
 */
...
/*
 위의 클래스를 상속하면 부모가 가진 추상메소드를 그대로 상속받게 되므로 해당 클래스도 asbstract로 선언해야 한다.
 단, 이경우에는 객체를 생성할 수 없으므로 부모의 추상메소드를 오버라이딩 해야한다.
 */
...
//		AbsClass1 absClass1 = new AbsClass1(); // -> 에러발생
		...
		/*
		추상클래스로 인스턴스 생성을 할수없지만, 참조변수로 사용하는것은 가능하다.
		 */
		AbsClass2 absClass2 = new AbsClassChild2();
		absClass2.absMethod(100);
		((AbsClassChild2)absClass2).newMethod();/*
		부모타입의 참조변수로 자식객체에 접근하려면 다운캐스팅후 접근해야한다.
		*/
		
		/*
		추상클래스내에서 선언된 정적멤버는 오버라이딩의 대상이 아니므로, static이 가진 기본규칙을
		따른다. 즉, 클래스명으로 접근하면 된다.
		 */
