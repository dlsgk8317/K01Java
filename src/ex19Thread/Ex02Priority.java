...
쓰래드의 우선순위
	...
	setPriority() : 쓰레드의 우선순위를 반환할때 사용
 */
...
class MessageSendingThread extends Thread
{
	...
	public MessageSendingThread(String str, int pri) {
		message = str;
		setPriority(pri); // setPriority 우선순위를 설정해준다.
	}
	
	public void run() {
		for(int i=1; i<=1000; i++) {
			System.out.println(message + i + "(" + getPriority() + ")"); //getPriority 우선순위를 반환
...
public class Ex02Priority {
	...
	public static void main(String[] args) {
		...
		2단계 실행
			...
			1~10사이에서 부여할 수 있고 높을수록 우선순위도 높아지게된다.
			※단, 우선순위가 높다고 무조건 먼저 실행되지는 않는다.
			전체적인 확률이 높아지게되면 먼제 실행될 가능성이 높아진다고 생각하면된다.
		 */
