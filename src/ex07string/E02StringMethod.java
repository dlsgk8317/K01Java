...
public class E02StringMethod {
	...
	public static void main(String[] args) {
		...
		/*
		 * 시나이로] 주민등록번호를 이용하여 성별을 판단하는 프로그램을 작성하시오 190419-3000000 => 남자 190419-4000000
		 * => 여자
		 */
		String juminNum = "190419-5000000";
		// 문자열에서 문자를 뽑아 비교하므로 싱클쿼테이션을 사용한다.
		...
		/*
		 * 4] compareTo() : 두 문자열의 첫번쨰 문자부터 순차적으로 비교하면서 앞의 문자열의 아스키코드가 크면 양수를 반환 뒷쪽 문자열의
		 * 아스키코드가 크면 음수를 반환 같으면 0을 반환한다.
		 */
		String str3 = "A";
		String str4 = "B";
		...
		/*
		 * 5] concat() : 두개의 문자열을 연결할때 사용한다. + 기호와 동일한 역할을 한다.ㄴ
		 */
		System.out.println("JAVA".concat(" WORLD").concat(" GO"));
		System.out.println("JAVA " + "WORLD " + "GO");

		/*
		 * 6] contains() : 문자열에 특정한 문자열이 포함되어 있는지를 판단하여 포함되었다면 true를 반환한다.
		 */
		System.out.println(str1.contains("TO"));
		System.out.println(str1.contains("to"));

		/*
		 * 시나리오] 해당 문자열이 이메일 형식인지 검사하는 프로그램을 작성하시오. hong@ikosmo.co.kr => 이메일형식임
		 * not@naver = > 이메일형식이 아님
		 */
...
//	String email1 = "hong@ikosmo.co.kr";
		...
		/*
		 * 7] endWith() : 문자열이 특정 문자열로 끝나면 true, 아니면 flase를 반환한다. 8] startsWith() : 특정
		 * 문자열로 시작하면 true를 반환한다.
		 */

		System.out.println("www.daum.net".endsWith("net")); // true
		System.out.println("naver.com".startsWith("http")); // flase

		/*
		 * 9] format() : 출력형식을 저장하여 문자열로 반환하고 싶을때 사용한다. printf()와 사용법은 동일하고, 주로
		 * 웹어플리케이션(JSP)를 제작할때 많이 사용된다.
		 */
		...
		/*
		 * 10] indexOf() : 문자열에서 특정 문자열의 시작인덱스를 변환한다. 만약 찾는 문자열이 없다면 -1을 반환한다.
		 */
		...
		/*
		 * 11] lastIndexOf() : 사용법은 indexOf()와 동일하나, 문자열의 뒤에서부터 문자열을 찾아서 인덱스를 변환한다.
		 */
		...
		/*
		 * 12] replace : 특정 문자열을 찾아서 지정된 문자열로 변경한다. 만약 찾는 문자열이 없다면 변경만 되지 않을뿐 에러가 발생하지는
		 * 않는다.
		 */
		...
		/*
		 * 13] split() : 문자열을 구분자를 통해 분리해서 String타입의 배열로 반환한다. 해당 구분자가 없는 경우에는 크기가 1인
		 * 배열로 변환한다.
		 */
