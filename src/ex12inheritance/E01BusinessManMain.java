...
/*
상속(Inheritance)
	: 기존에 정의된 클래스에서 메소드와 변수를 추가하여 새로운
	클래스를 정의하는것을 말한다.
	부모클래스를 자식클래스가 상속할때는 extends 키워드를
	사용한다.
 */

//부모클래스(상위, 기본, super) : 사람의 일반적은 형태를 추상화 한 클래스.
class Man {
	private String name;
	...
	public Man(String name) {
		this.name = name;
		System.out.println("Man 클래스 인자 생성과 호출됨");
	}
	public void tellYourName() {
		System.out.println("내 이름은" + name + "입니다.");
	}
}
//자식클래스(하위, 유로 sub) : Man클래스를 상속받는 회사원 클래스
class BusinessMan extends Man {
	...
	/*
	상속받은 자식클래스에는 부모클래스의 생성자를 호출하여 부모객체가
	먼저 생성도도록 해야한다.
