...
public class AnimalDog extends Animal {
	...
	//확장한 멤버변수
	String dogking; //강아지 종류
	String name; //이름
	
	/*
	부모클래스의 species 멤버변수는 private으로 선언되어 자식에서
	접근이 불가능하므로, 이때는 public으로 선언된 getter 메소드를
	통해접근해야 한다.
	 */
	void bark() {
		System.out.printf("\n이름은 %s이고 종이 %s인 강아지가 짖는다.\n", name, getSpecies());
	}
	void showDog() {
		/*
		 ...
		 */
		super.showAnimal();
		System.out.println("종류는:" + dogking);
	...
	public AnimalDog(String species, int age, String gender, String dogkind, String name) {
		/*
		자식 클래스에서 반드시 부모클래스의 객체를 먼저 생성해야 한다.
