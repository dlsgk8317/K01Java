...
다형성(Polymorphism)
	...
	- 부모의 참조변수로 자식 객체를 참조
	1. 이경우 부모타입의 참조변수는 부모로부터 상속받는 
		멤버까지만 접근가능하다.
	2. 자식에서 오버라이딩한 메소드가 우선 호출된다.
	3. 자식에서 새롭게 정의한 멤버에는 접근할 수 없다.
	4. 이런경우는 이질화(Heterogeneous:혜태로지니어스)라고 한다.
	
	- 같은 타입의 클래스로 객체를 참조ㅓ
	1. 객체 전체를 접근할 수 있다. 즉 클래스의 일반적인
		규칙이 적용된다.
	2. 이런경우는 동질화(Homegeneous:호모지니어스)라고한다.
 */
...
public class E11Polymorphism {
	...
	public static void main(String[] args) {
		...
		이질화 : 부모타입의 변수로 자식 객체를 참조하는것.
			이경우 자식에서 새롭게 정의한 멤버에서 접근할수 없다.
			만약 접근해야 한다면 자식타입으로 강제형변환(다운케스팅)
			해야 한다.
		 */
		System.out.println("[부모타입에 자식타입의" + "메모리 주소복사]");
		
		Parent parent1 = homeChild;
		parent1.parentMember = 1;
//		parent1.childMember = 1; // 접근불가 -> 부모타입으로 자식멤버에 접근할수 없음.
		...
		따라서 모든 객체는 Object의 참조변수로 참조가능하다.
