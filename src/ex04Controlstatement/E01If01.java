...
public class E01If01 {
	...
	if문 (조건문, 분기분)
		...
		- if문의 조건문은 반드시 비교식 혹은 논리식 이여야 한다.
		- 즉 결과값이 boolean타입(true, false)이여야 한다.
		- 실행할 문장이 하나인경우 중괄호 생략이 가능하다.
	 */

	public static void main(String[] args) throws IOException { //throws 무시한다.
...
//		}
		...
		if(num%2 != 0); // -> 에러는 발생되지않으나, if문이 종료되므로 조건과상관없이 출력
		{
		...
		system.in.read()
			...
			해당 함수는 IOException을 ㄱ본적으로 가지고있어
			이를 무시하기 위해 main()메소드에서 throws 처리를 해준다.
		 */
		
//		System.out.print("하나의 문자를 입력하세요:");
//		int asciiCode = System.in.read(); //read 하나의 문자를 입력받을때 사용하는 함수
...
//		System.out.println("입력한문자:" + (char)asciiCode);
		...
		/*
		 아스키코드값을 알고있을때의 구현방법
		 '0' => 48
		 '9' => 57 이므로 해당 구간안에 있다면 숫자로 판단할수있디.
