...
public class E06BreakContinue {
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			조건문과 함께 사용해야 한다. 그렇지 않으면 Unreachable code 에러가 발생한다.
			 */
			System.out.printf("[i가 %d일때]\n", i);
			
			System.out.println("continum이전 출력문");
			if(i%2==0)continue; // 조건부없이 사용하면 에러발생
			System.out.println("continum이후 출력문");
			
			System.out.println("break이전 출력문");
			if(i==3) break; // 조건부없이 사용하면 에러발생
			System.out.println("break이후 출력문");			
		}
		
		/*
		중첩된 반복문 안에서 break문을 만나면 '가장 가까운 반복문 하나'를 탈출하게된다.
		 */
