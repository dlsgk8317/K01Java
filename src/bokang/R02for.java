...
public class R02for {
	...
	public static void main(String[] args) {
		...
		//시나리오] 1~10까지 합을 구하는 while문을 작성하시오.
		...
		do {
			//복합대입연산자는 좌측 우측항을 연산한후 좌측항에 대입한다.
			total += b;
			b++;
		}while(b<=10);
...
//					System.out.println(i+"+"+j+"="+(i+j));
		 ...
		 */
		System.out.println("방정식을 만족하는 h,j를 for문으로 찾기");
		
		for(int h=0; h<=10; h++) {
			for(int j=0; j<=10; j++) {
				if((2*h)+(4*j) == 12) {
					System.out.printf("2x+4y=12를" + "만족하는 h,j:%d,%d\n",h,j);
				}
			}
		}
		...
		for(int s=1; s<=10; s++) {
			totalsum2 += s; //한번누적한값을
			totalsum3 += totalsum2; //한번더 누적해서 더함
