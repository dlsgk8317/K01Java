...
public class E01JunsuType {
	...
	public static void main(String[] args) {
		...
		/*
		 정수형자료
		 -byte, sort int, long 타입(형)이 있다.
		 - 각자료의 표현범위는 교안을 참고한다.
		 - 일반적으로 cpu(중앙처리장치)는 정수형 연산을 할 때 int형을 가장 빠르게 처리를 한다.
		...
		System.out.println("int1 + int2 =" + result1); // 결과값 : 300
		...
		*/
		...
		- byte, short형은 게임캐릭터의 움직임의 표현이나 음원정보등을 저장할때 주로 사용한다.
		*/
		...
		/*
		- 동일자료형끼리의 연산은 동일자료형이 되는것이 기본적이다.
		- 큰 자료형과 작은 자료형을 연산하면 큰 자료형으로 자동형변환된다.
		(데이터의 손실이 있을수 있기때문)
		- 단, byte와 short는 에외적인 상황으로 생각하면된다.(int형에 최적화된 cpu의 특성때문에)
		*/
		...
		result4 = long1 + long2; //long끼리의 연산이므로, 기본규칙에 따름
		System.out.println("int1 + long2 =" + result4);
		/*
		자바컴파일러는 정수형상수를 기본적으로 int형으로 간주하기때문에
		위 정수를 메모리에 올리는 순간 에러가 발생한다.
		이때에는 접미사(L or l)을 붙여서 int형 자료가 아닌 long형 자료임을 
		알려줘야한다. 이와같이 대입을 위해 입력되는 숫짜도 자료형을 기반으로
