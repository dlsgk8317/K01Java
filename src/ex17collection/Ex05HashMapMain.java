...
public class Ex05HashMapMain {
	...
	System.out.println("name이라는 키값으로 저장된 이전의값:" + map.put("name", "홍길동"));//->null 반환됨
...
//	map.put("age", number);//[에러발생].value타입이 맞지않음.
	...
	/*
	3.객체수 : Map에 저장된 객체수 반환
	 */
	System.out.println("저장된 객체수:" + map.size()); //4개
	
	/*
	4.중복저장 : 기존에 입력된 name키값이 있으므로 "홍길동"이 출력된다.
	...
	5-1]키값을 알고 있을때 출력하기
		: get(키값)으로 출력한다.
	 */
	System.out.println("키캆을 알때:" + map.get("name"));
	
	/*
	5-2]키값을 모를떄 출력하기
		: keySet() 메소드로 호출하여 키값을 Set컬렉션으로 얻어온다.
		해당 반환값을 확장for문을 통해 추출하고, 이를 통해 값을 출력할수있다.
	...
	/*
	5-3] Iterator(반복자)를 통한 출력
		:Iterator를 통해 출력할때도 keySet()을 통해 먼저 
		key값을 얻어온후 반복한다.
	 */
	...
	/*
	5-4] Value값만 얻어와야 할때 : values() 메소드 사용
	 */
	...
	/*
	7] 삭제 : key를 통해 삭제하고, 삭제되면 해당 value가 반환된다.
	 */
