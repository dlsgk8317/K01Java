...
class FruitSeller2 {
	...
	상수는 초기값이 없는 형태로 선언 자체가 불가능하다.
	 */

	
	/*
	apple_price를 상수에서 변수로 변경한 이유는 상수는 단 한번만
	초기화되므로 멤버메소드에서는 초기화할 수 없다. 멤버메소드는 개발자가
	원할떄 언제든지 호출할 수 있으므로, 상수의 특성과는 맞지 않기 때문이다.
	 */
...
class FruitBuyer2 {
	...
	/*
	 * 구매자가 판매자에게 사과를 구매하는 행위를 표현한 멤버메소드 1. 판매자에게 금액을 지불한다.(saleApple을 호출한다.) ==
	 * 여기에서 seller객체가 saleApple() 메소드가 호출된다. == 2. 판매자가 금액에 해당하는 사과를 갯수를 반환하면, 해당
	 * 갯수만큼 보유한 사과의 갯수를 증가시킨다. 3.판매자에게 지불한 금액만큼 차감된다.
	 */
