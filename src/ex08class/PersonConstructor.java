...
/*
생성자(Contructor)
- 클래스를 객체화(인스턴스화)할떄 자동으로 호출되는 메소드
- 자동으로 호출되며, 개발자가 임의로 호출할 수 없다
- 반환값이 없다. 즉, 특정 연산후 값을 반환할 수 없다.
따라서 void를 쓰지않는다.
- 그 외는 메소드가 가진 모든 특성을 가지고있다.(인자전달, 오버로딩 등)
...
용도2 : 생성자 내에서 멤버변수와 매개변수의 이름이 동일할때 
	...
	this.변수명 -> 멤버변수
	변수 -> 생성자의 매개변수
	단, 이름이 서로 다르면 this를 생략할 수 있다. 주로 
	명시적인 코드작성을 위해 사용된다.
 */
...
public class PersonConstructor {
	...
	// 생성자는 클래스와 이름이 똑같아야하며, 반환타입이 없어야한다.
	//생성자1 : 기본생성자
	public PersonConstructor() {
		name = "이름없음";
		age = 0;
...
//		this(name, age, "미상"); -> 에러발생됨.
	...
	void showPersonInfo() {
		System.out.printf("%s님의 정보\n", this.name); //명시적으로 멤버변수인걸 알려주기위해서 사용
