...
class HighFriend extends Friend {
	...
	super를 통해 호출하고, 자식에서 확장한 변수를 별도의 print문을 통해 출력하고있다.
	 */
	@Override
	public void showAllData() {
...
public class E13MyFriendSeriallzable {
	...
	호출하는 정도로만 구성한다. main메소드는 단지 프로그램의 시작점 정도의 
	의미만 가지는게 좋다.
	 */
	public static void main(String[] args) {
		
		//프로그램의 비지니스 로직을 가진 핸들러 객체를 생성한다.
...
class FriendInfoHandler{
	...
	public void addFriend(int choice) {
		Scanner scan = new Scanner(System.in);
			...
			/*
			1. 친구정보를 입력받은 후
			2. 객체배열의 0번방에 객체를 저자하고.
			3. numOfFriends를 1 증가시킨다. 후위증가이므로 이렇게 처리된다.
			 */
			if(choice==1) { // 고딩친구 입력후 추가
	...
	3. 정보 출력시 배열에 입력된 객체수만큼 반복하면서 각 요소를 통해 멤버메소드를 호출한다.
	4. 이때 오버라이딩된 메소드를 통해 항상 해당 객체의 정보를 출력할  수 있다.
		참조변수의 타입에 영향을 받지 않고 항상 오버라이딩된 최하위의 메소드가 호출된다.ㄴㄴㄴㄴㄴㄴㄴㄴㄴㄴㄴ
	 */
	...
	public void searchInfo() {
		...
		for(int i=0; i<numOfFriends; i++) {
			/*
			검색을 위한 입력한 이름과, 각 인덱스에 저장된 개게의 name변수의
			비교를 통해 문자열검색을 진행한다.
			 */
	...
	public void deleteInfo() {
		...
		if(deleteIndex==-1 ) {
			...
			/*
			객체배열에서 검색된 요소를 작제한 후 입력된 위치의 바로뒤
			요소를 앞으로 하나씩 이동시킨다.
			 */
